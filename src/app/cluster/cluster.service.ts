...
export class ClusterService {
  ...
  get clusters() {
    const items = (this.storage.get<Cluster[]>(STORAGE_KEY_CLUSTERS) || defaultClusters).sort((a, b) => a.name > b.name ? 1 : -1)
    const cluster = this.storage.get<Cluster>(STORAGE_KEY_CLUSTER) || defaultClusters[0]
    return items.map(i => ({
      ...i,
  ...
  deleteCluster(cluster: Cluster) {
    this.storage.set(STORAGE_KEY_CLUSTERS, [...this.clusters.filter(c => c.name !== cluster.name)])
  }
  ...
  getExplorerUrl(path: string) {
    return `https://explorer.solana.com/${path}${getClusterUrlParam(this.cluster)}`
  }
}
